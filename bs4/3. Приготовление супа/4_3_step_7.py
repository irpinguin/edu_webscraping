"""
Написать код, который будет обрабатывать HTML-структуру, состоящую из тегов <p>.
Код должен анализировать текст внутри каждого тега и, если количество символов в
тексте (без учета пробелов) является чётным, суммировать значения атрибутов id и class.
"""
...
def sum_even_length_ids(html):
    ...
    for item in p_tags:
        p_tag_text_len = len(item.get_text(strip=True).replace(' ', ''))
        if p_tag_text_len % 2 == 0:
            if item.get('id'):
                summ_id = int(item['id'])
                total_id_sum += summ_id
            if item.get('class'):
                summ_class = sum(int(class_item) for class_item in item.get('class') if class_item.isdigit())
                total_class_sum += summ_class

        print(f'{summ_id}, {summ_class}, {item.text}')
