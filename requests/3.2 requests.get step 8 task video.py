...
def main():
    ...
    try:
        response = requests.get(url=URL+FILENAME_SRC, timeout=TIMEOUT, stream=True)
    ...
    try:
        with open(FILENAME_DEST, 'wb') as video_file, tqdm(
                desc=f"Загрузка {FILENAME_SRC}",
                total=remote_file_size(URL + FILENAME_SRC),
                unit='B',
                unit_scale=True,
                unit_divisor=1024,
...
#   не нашел как сделать так, чтобы в прогресс-баре размер файла выводился с точностью до 2х знаков
        ...
        print(f"Ошибка: Нет прав на запись в {FILENAME_DEST}.")
    except OSError as e:
        ...
        remote_size = remote_file_size(URL + FILENAME_SRC)
        local_size = os.path.getsize(FILENAME_DEST)
        try:
